...
class Dapur extends BaseController
{
    ...
    public function index()
    {
        ...
        if (session()->get('role_id') == 1 or session()->get('role_id') == 3) {

            $db = \Config\Database::connect();
            $builder = $db->table('penjualan');
            $builder->select('penjualan.id,invoice,pelanggan,tanggal,nomor_meja');
        ...
        } else  if (session()->get('role_id') == 1 or session()->get('role_id') == 5) {
